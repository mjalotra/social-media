...
public interface PostJpaRepository extends JpaRepository<Post, Long> {
	...
	/**
	 * Method that feches a list of top 20 post or feeds to be visible on user's
	 * page based upon their interest of followees.
	 * 
	 * @param userId
	 * @return List<Post>
	 */
	@Query(value = "select p.*, u.name from post p "
			...
			+ " inner join user u on p.posted_by = u.id " + "where uf.user_id= :userId "
